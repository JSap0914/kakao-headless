...
8. Inspect `receipt PREVIEW_UUID`. `unknown` or `accepted_unverified` is never permission to retry. Read history, identify the exact own-message log ID, then use `reconcile PREVIEW_UUID --log-id LOG_ID --ack-risk` for read-only verification. The original receipt is preserved and separate verification evidence is added. Never delete reservations or create new previews automatically to work around a send error.
